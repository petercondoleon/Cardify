...
#pragma warning restore 618
        ...
        {
            // Card must be spawned in the server so a networking Command is used.
            CardsInHand -= 1;
            if (CardsInHand < 0)
            {
                CardsInHand = CardImages.Length;
            ...
            if (CardsInHand == CardImages.Length)
            {
                for (int i = 0; i < CardImages.Length; i++)
                {
                    CardImages[i].gameObject.SetActive(true);
            ...
            {
                CardImages[CardsInHand].gameObject.SetActive(false);
            }
